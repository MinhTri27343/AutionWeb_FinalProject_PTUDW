...
const OrderInfoCard = ({ product, order }: ComponentProps) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm w-full transition-all hover:border-teal-200">
      {/* Phần thân trên: Đã thêm min-w-0 để xử lý tên dài */}
      <div className="p-5 flex items-center gap-4">
        ...
        <div className="flex flex-col min-w-0">
          {" "}
          {/* min-w-0 giúp con bên trong có thể truncate */}
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <h3 className="font-bold text-slate-900 text-lg leading-tight truncate">
              {order.buyer.name}
            </h3>
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-teal-50 border border-teal-100 text-[10px] font-bold text-teal-600 uppercase tracking-wider shrink-0">
              <UserCheck className="w-3 h-3 fill-teal-600/10" />
              Người mua
            </span>
          </div>
      ...
      {/* Phần thân dưới: Tự động xuống hàng khi màn hình quá nhỏ (<450px) */}
